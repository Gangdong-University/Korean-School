...
function AdminPanel({students,setStudents,currentTeacherId,onClose}){
  ...
  const saveTeacher=()=>{
    ...
    if(editTeacherId){
      ...
      if(idx!==-1)_teachers[idx]={..._teachers[idx],name:tf.name.trim(),email:emLow,role:tf.role,class_ids:tf.class_ids,...(tf.password.length>=6?{password:tf.password}:{})};
    } else {
      _teachers.push({id:`t${Date.now()}`,name:tf.name.trim(),email:emLow,password:tf.password,role:tf.role,class_ids:tf.class_ids});
    }
...
function ClassDetail({cls,isAdmin,isSuperAdmin,students,setStudents,goBack,attMonth,setAttMonth,badgeDefs,setBadgeDefs}){
  ...
  const addVocabEntry=()=>{
    ...
    _db.vocab_entries.push({id:`ve${Date.now()}`,class_id:cls.id,month:vocabMonth,word:vocabWord.trim(),meaning:vocabMean.trim(),type:vocabType});
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,flexWrap:"wrap"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            {editName?(
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                ...
                <button onClick={()=>{_db.classes=_db.classes.map(c=>c.id===cls.id?{...c,name:nameVal}:c);setEditName(false);}} style={bs("#7c3aed","#fff",undefined,true)}>✓</button>
          ...
          {isAdmin&&(
            <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
              ...
              {isSuperAdmin&&<button onClick={()=>{if(window.confirm(`"${cls.name}" устгах уу?`)){_db.classes=_db.classes.filter(c=>c.id!==cls.id);goBack();}}} style={bs("#fff0f0","#e53935","#ffcdd2",true)}>🗑️</button>}
            </div>
          )}
        </div>
        ...
        {isAdmin&&(
          <div style={{background:"#fff",borderRadius:12,padding:10,marginBottom:12,display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
            ...
            {[1,2,3,4,5,6,7].map(d=>{
              ...
              return(
                <button key={d} onClick={()=>{const nd=active?classDays.filter(x=>x!==d):[...classDays,d].sort();setClassDays(nd);_db.classes=_db.classes.map(c=>c.id===cls.id?{...c,days:nd}:c);}}
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                  {vocabEntries.filter(v=>v.month===mo).map(v=>(
                    <div key={v.id} style={{display:"flex",alignItems:"center",gap:4,background:v.type==="vocab"?"#f3e8ff":"#e8f5e9",border:`1px solid ${v.type==="vocab"?"#a78bfa":"#66bb6a"}`,borderRadius:8,padding:"3px 8px",fontSize:11}}>
                      ...
                      <span onClick={()=>{_db.vocab_entries=_db.vocab_entries.filter(x=>x.id!==v.id);setTick(t=>t+1);}} style={{cursor:"pointer",opacity:.4,fontSize:9}}>✕</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <Overlay onClose={()=>setConfirmDel(null)}>
            <div style={{textAlign:"center"}}>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button onClick={()=>{_db.students=_db.students.filter(s=>s.id!==confirmDel);setStudents(p=>p.filter(s=>s.id!==confirmDel));setConfirmDel(null);}}
  ...
  useEffect(()=>{
    async function loadAll(){
      try{
        ...
        const [cls,sts,bds,voc,pays,pends]=await Promise.all([r1.json(),r2.json(),r3.json(),r4.json(),r5.json(),r6.json()]);
        _db.classes=cls||[];
          ...
          <Overlay onClose={()=>setShowAdd(false)} maxW={340}>
            ...
            <div style={{display:"flex",gap:7}}>
              ...
              <button onClick={()=>{
                if(!nc.name)return;
                const newCls={id:`c${Date.now()}`,name:nc.name,time:nc.time,days:nc.days,color:nc.color};
                _db.classes.push(newCls);setClasses(p=>[...p,newCls]);
                setNc({name:"",time:"",days:[],color:"#e91e8c"});setShowAdd(false);
              }} style={{...bs("#7c3aed","#fff"),flex:1,justifyContent:"center"}}>Үүсгэх</button>
            </div>
          </Overlay>
        )}
